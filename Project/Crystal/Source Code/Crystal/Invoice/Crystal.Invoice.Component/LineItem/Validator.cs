...
namespace Crystal.Invoice.Component.LineItem
{
    public class Validator : BinAff.Core.Validator
    {
        ...
        protected override List<Message> Validate()
        {
            ...
            if (!ValidationRule.IsNullOrEmpty(data.Start) && ValidationRule.IsDateGreaterThanToday(data.Start))
                retMsg.Add(new Message("Start date cannot be greater than today.", Message.Type.Error));

            if (!ValidationRule.IsNullOrEmpty(data.End) && ValidationRule.IsDateGreaterThanToday(data.End))
